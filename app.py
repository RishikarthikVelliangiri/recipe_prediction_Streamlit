...
FEATURE_PLOTS_DIR = 'feature_importance_plots' # <<< New: Path to importance plots
SCORE_THRESHOLD = 200
...
# Expected filenames for the importance plots
PLOT_FILENAME_ALL = "importance_grid_all_features.png"
PLOT_FILENAME_NO_DOWN = "importance_grid_no_thumbs_down.png"
PLOT_FILENAME_NO_BOTH = "importance_grid_no_thumbs_both.png"


# --- Helper Functions ---
# clean_category_name_for_model_files, format_category_for_wordcloud_path, load_model, get_wordcloud_paths
# (These functions remain the same as the previous version - keep them here)
...
def load_model(category_name, mode):
    ...
    except Exception as e: # Catch-all for other load errors
...
if selected_category:
    ...
    plot_path_no_down = os.path.join(FEATURE_PLOTS_DIR, PLOT_FILENAME_NO_DOWN)
    ...
    # Display Plot 2: No Thumbs Down
    st.markdown("**2. Importance: No Thumbs Down Feature**")
    if os.path.exists(plot_path_no_down):
        st.image(plot_path_no_down, caption="Feature importance grid excluding 'thumbs_down'.")
    else:
        st.warning(f"Plot file not found: '{plot_path_no_down}'. Please run the 'generate_importance_plots.py' script.")
